...
const single = async (pageId: number, cookie: string) => {
  const homes = await limit(() => getNewestHomes(cookie, pageId)).catch(
    (e: Error) => e,
  );
