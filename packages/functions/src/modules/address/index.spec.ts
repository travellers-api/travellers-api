...
describe('generateHomeIds', () => {
  ...
  describe('件数が間隔の数値より少ない場合', () => {
    test('0:00 15件 30分間隔', () => {
      const now = dayjs('2023-06-15T00:00:00+09:00').tz('Asia/Tokyo');
      expect(generateHomeIds(now, 15, 30)).toEqual([1]);
    });

    test('0:01 15件 30分間隔', () => {
      const now = dayjs('2023-06-15T00:01:00+09:00').tz('Asia/Tokyo');
      expect(generateHomeIds(now, 15, 30)).toEqual([2]);
    });

    test('0:02 15件 30分間隔', () => {
      const now = dayjs('2023-06-15T00:02:00+09:00').tz('Asia/Tokyo');
      expect(generateHomeIds(now, 15, 30)).toEqual([3]);
    });

    test('0:15 15件 30分間隔', () => {
      const now = dayjs('2023-06-15T00:15:00+09:00').tz('Asia/Tokyo');
      expect(generateHomeIds(now, 15, 30)).toEqual([]);
    });
  });
});
