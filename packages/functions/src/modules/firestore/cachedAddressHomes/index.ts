...
export const getHomes = async (): Promise<{ id: number; data: CachedAddressHome }[]> => {
  const querySnapshot = (await collection.get()) as QuerySnapshot<CachedAddressHome>;
...
export const getHome = async (id: number): Promise<CachedAddressHome | null> => {
  const snapshot = (await collection.doc(id.toString()).get()) as DocumentSnapshot<CachedAddressHome>;
  const data = snapshot.data();
  if (!data) return null;
  return data as CachedAddressHome;
};
