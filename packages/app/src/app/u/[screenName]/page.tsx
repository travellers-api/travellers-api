...
const getData = async (screenName: string): Promise<Props> => {
  const [address, hafh] = await Promise.all([
    ...
    fetch(
      `https://api.travellers-api.amon.dev/hafh/users/${screenName}/reservations`,
      {
        next: { revalidate: 60 * 60 },
      },
    ).then(async (res) => {
  ...
  const props: Props = {
    ...
    reservations: [
      ...
      .filter(
        (reservation) =>
          reservation.data.status === "approved" ||
          reservation.data.status === "staying",
      ),
  };
                ...
                <div className="font-sans text-xs font-bold text-black/50">
                  <p>
                    <time dateTime={reservation.data.checkInDate}>
                      {dayjs
                        .tz(reservation.data.checkInDate, "Asia/Tokyo")
                        .format("MM.DD ddd")}
                      -
                    </time>
                  </p>
                  <p>
                    <time dateTime={reservation.data.checkOutDate}>
                      {dayjs
                        .tz(reservation.data.checkOutDate, "Asia/Tokyo")
                        .format("MM.DD ddd")}
                    </time>
                  </p>
                </div>
                <div className="grid grid-cols-1 content-start gap-8 border-l-2 pl-20">
                  <p className="line-clamp-1 font-sans text-3xl font-bold opacity-80">
                    {reservation.data.home.name}
                  </p>
                  <div className="flex gap-8">
                    <p className="font-sans text-xs font-bold text-black/50">
                      {reservation.service}
                    </p>
                  </div>
                </div>
