...
const getData = async (screenName: string): Promise<Props> => {
  const [address, circle, hafh] = await Promise.all([
    fetch(`https://api.travellers-api.dev/address/users/${screenName}/reservations`, {
    ...
    fetch(`https://api.travellers-api.dev/circle/users/${screenName}/reservations`, {
      next: { revalidate: 60 * 60 },
    }).then(async (res) => {
      const { reservations } = res.ok
    ...
    fetch(`https://api.travellers-api.dev/hafh/users/${screenName}/reservations`, {
      next: { revalidate: 60 * 60 },
    }).then(async (res) => {
      const { reservations } = res.ok
